#!/usr/bin/env python3
# exp_dynamic: performance on different dynamic workload patterns.
...
for tmp_round_index in round_indexes:
    tmp_log_dirpath = "{}/exp_dynamic/round{}".format(Common.output_log_dirpath, tmp_round_index)
    ...
    for tmp_cache_name in cache_names:
        ...
        # Run prototype for each dynamic pattern
        for tmp_pattern in pattern_list:
            tmp_log_filepath = "{}/tmp_evaluator_for_{}_{}.out".format(tmp_log_dirpath, tmp_cache_name, tmp_pattern)
            SubprocessUtil.tryToCreateDirectory(Common.scriptname, os.path.dirname(tmp_log_filepath))

            # Check log filepath
            if os.path.exists(tmp_log_filepath):
                LogUtil.prompt(Common.scriptname, "Log filepath {} already exists, skip {} w/ {} dynamic pattern for the current round {}...".format(tmp_log_filepath, tmp_cache_name, tmp_pattern, tmp_round_index))
                continue
            ...
            tmp_exp_settings["workload_pattern_name"] = tmp_pattern
            tmp_exp_settings["cache_name"] = tmp_cache_name

            # Launch prototype
            LogUtil.prompt(Common.scriptname, "Run prototype of {} w/ {} dynamic pattern for the current round {}...".format(tmp_cache_name, tmp_pattern, tmp_round_index))
