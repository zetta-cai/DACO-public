#!/usr/bin/env python3
# exp_performance_perclient_distopcnt: parameter analysis on different operation count to represent workload distribution (NOT change workload distribution and workload sequence; just affect workload distribution representation precision) in each cache node.
...
cache_names = ["covered", "gdsf+", "lhd+", "gdsf", "lhd"]
# NOTE: NO need to run 1M, which is the same as previous experiments (performance against existing methods and extended methods)
perclient_distopcnt = [10000000, 100000000] # 10M and 100M

# Run the experiments with multiple rounds
for tmp_round_index in round_indexes:
    tmp_log_dirpath = "{}/exp_parameter_perclient_distopcnt/round{}".format(Common.output_log_dirpath, tmp_round_index)
    ...
    for tmp_cache_name in cache_names:
        ...
        # Run prototype for each per-edge opcnt for distribution precision
        for tmp_perclient_opcnt in perclient_distopcnt:
            tmp_log_filepath = "{}/tmp_evaluator_for_{}_{}.out".format(tmp_log_dirpath, tmp_cache_name, tmp_perclient_opcnt)
            SubprocessUtil.tryToCreateDirectory(Common.scriptname, os.path.dirname(tmp_log_filepath))

            # Check log filepath
            if os.path.exists(tmp_log_filepath):
                LogUtil.prompt(Common.scriptname, "Log filepath {} already exists, skip {} w/ per-client distopcnt {} for the current round {}...".format(tmp_log_filepath, tmp_cache_name, tmp_perclient_opcnt, tmp_round_index))
                continue
            ...
            tmp_exp_settings["perclient_opcnt"] = tmp_perclient_opcnt
            tmp_exp_settings["cache_name"] = tmp_cache_name

            # Launch prototype
            LogUtil.prompt(Common.scriptname, "Run prototype of {} w/ per-client distopcnt {} for the current round {}...".format(tmp_cache_name, tmp_perclient_opcnt, tmp_round_index))
