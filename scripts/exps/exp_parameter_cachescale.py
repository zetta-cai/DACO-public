#!/usr/bin/env python3
# exp_performance_cachescale: parameter analysis on different cache scales (# of cache nodes).
...
# NOTE: NO need to run 4 nodes, which is the same as previous experiments (performance against existing methods and extended methods)
edgecnt_list = [8, 12, 16]

# Run the experiments with multiple rounds
for tmp_round_index in round_indexes:
    tmp_log_dirpath = "{}/exp_parameter_cachescale/round{}".format(Common.output_log_dirpath, tmp_round_index)
    ...
    for tmp_cache_name in cache_names:
        ...
        # Run prototype for each cache scale
        for tmp_edgecnt in edgecnt_list:
            ...
            # Launch prototype
            LogUtil.prompt(Common.scriptname, "Run prototype of {} w/ {} edgecnt for the current round {}...".format(tmp_cache_name, tmp_edgecnt, tmp_round_index))
