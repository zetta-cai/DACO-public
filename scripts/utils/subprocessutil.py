    ...
    def tryToCreateDirectory(scriptname, dirpath, keep_silent=False):
        if not os.path.exists(dirpath):
            ...
            if tmp_mkdirs_subprocess.returncode != 0:
                LogUtil.die(scriptname, "failed to create directory {} (errmsg: {})".format(dirpath, SubprocessUtil.getSubprocessErrstr(tmp_mkdirs_subprocess)))
    ...
    def moveSrcToTarget(scriptname, srcpath, targetpath, keep_silent=False):
        if os.path.exists(srcpath):
            ...
            if tmp_move_subprocess.returncode != 0:
                LogUtil.die(scriptname, "failed to move {} to {} (errmsg: {})".format(srcpath, targetpath, SubprocessUtil.getSubprocessErrstr(tmp_move_subprocess)))
        else:
            LogUtil.die(scriptname, "The source path {} does not exist".format(srcpath))
