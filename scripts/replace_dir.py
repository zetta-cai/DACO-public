...
def replace_dir(original_dir, current_dir, path):
    if not os.path.exists(path):
        print("Path does not exist: " + path)
        return
    
    replace_rootdir_cmd = "sed -i 's!{}!{}!g' {}".format(original_dir, current_dir, path)
    replace_rootdir_subprocess = subprocess.run(replace_rootdir_cmd, shell=True)
    if replace_rootdir_subprocess.returncode != 0:
        print("Failed to replace rootdir: " + path)
        sys.exit(1)
...
replace_dir(default_facebook_config_filepath, current_facebook_config_filepath, "config.json")
