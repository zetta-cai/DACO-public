...
namespace covered
{
    ...
    CoveredEdgeWrapper::CoveredEdgeWrapper(const std::string& cache_name, const uint64_t& capacity_bytes, const uint32_t& edge_idx, const uint32_t& edgecnt, const std::string& hash_name, const uint32_t& keycnt, const uint64_t& local_uncached_capacity_bytes, const uint64_t& local_uncached_lru_bytes, const uint32_t& percacheserver_workercnt, const uint32_t& peredge_synced_victimcnt, const uint32_t& peredge_monitored_victimsetcnt, const uint64_t& popularity_aggregation_capacity_bytes, const double& popularity_collection_change_ratio, const CLILatencyInfo& cli_latency_info, const uint32_t& topk_edgecnt, const std::string& realnet_option, const std::string& realnet_expname) : EdgeWrapperBase(cache_name, capacity_bytes, edge_idx, edgecnt, hash_name, keycnt, local_uncached_capacity_bytes, local_uncached_lru_bytes, percacheserver_workercnt, peredge_synced_victimcnt, peredge_monitored_victimsetcnt, popularity_aggregation_capacity_bytes, popularity_collection_change_ratio, cli_latency_info, topk_edgecnt, realnet_option, realnet_expname), topk_edgecnt_for_placement_(topk_edgecnt), weight_tuner_(edge_idx, edgecnt, cli_latency_info.getPropagationLatencyClientedgeAvgUs(), cli_latency_info.getPropagationLatencyCrossedgeAvgUs(), cli_latency_info.getPropagationLatencyEdgecloudAvgUs())
    {
