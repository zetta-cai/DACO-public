...
namespace covered
{
    ...
    private:
        ...
        //bool nonblockDataFetchForPlacementInternal_(const Key& key, const Edgeset& best_placement_edgeset, const NetworkAddr& source_addr, UdpMsgSocketServer* recvrsp_socket_server_ptr, const ExtraCommonMsghdr& extra_common_msghdr, const bool& sender_is_beacon, bool& need_hybrid_fetching) const; // (OBSELETE for non-blocking placement deployment) Fetch data from local cache or neighbor to trigger non-blocking placement notification; need_hybrid_fetching indicates if we need hybrid fetching (i.e., resort sender to fetch data from cloud); return if edge is finished
        void nonblockDataFetchForPlacementInternal_(const Key& key, const Edgeset& best_placement_edgeset, const ExtraCommonMsghdr& extra_common_msghdr, const bool& sender_is_beacon, bool& need_hybrid_fetching) const; // Fetch data from local cache or neighbor to trigger non-blocking placement notification; need_hybrid_fetching indicates if we need hybrid fetching (i.e., resort sender to fetch data from cloud)
        void nonblockDataFetchFromCloudForPlacementInternal_(const Key& key, const Edgeset& best_placement_edgeset, const ExtraCommonMsghdr& extra_common_msghdr) const; // Fetch data from cloud without hybrid data fetching (a corner case) (ONLY invoked by edge beacon server instead of cache server of the beacon edge node)
        //bool nonblockNotifyForPlacementInternal_(const Key& key, const Value& value, const Edgeset& best_placement_edgeset, const NetworkAddr& source_addr, UdpMsgSocketServer* recvrsp_socket_server_ptr, const ExtraCommonMsghdr& extra_common_msghdr) const; // (OBSELETE for non-blocking placement deployment) Notify all edges in best_placement_edgeset to admit key-value pair into their local edge cache; return if edge is finished
        void nonblockNotifyForPlacementInternal_(const Key& key, const Value& value, const Edgeset& best_placement_edgeset, const ExtraCommonMsghdr& extra_common_msghdr) const; // Notify all edges in best_placement_edgeset to admit key-value pair into their local edge cache

        // (7.3) For beacon-based cached metadata update (non-blocking notification-based)
        void processMetadataUpdateRequirementInternal_(const Key& key, const MetadataUpdateRequirement& metadata_update_requirement, const ExtraCommonMsghdr& extra_common_msghdr) const;
        ...
        bool afterDirectoryLookupHelperInternal_(const Key& key, const uint32_t& source_edge_idx, const CollectedPopularity& collected_popularity, const bool& is_global_cached, const bool& is_source_cached, Edgeset& best_placement_edgeset, bool& need_hybrid_fetching, FastPathHint* fast_path_hint_ptr, UdpMsgSocketServer* recvrsp_socket_server_ptr, const NetworkAddr& recvrsp_source_addr, BandwidthUsage& total_bandwidth_usage, EventList& event_list, const ExtraCommonMsghdr& extra_common_msghdr) const; // Return if edge is finished
        void afterDirectoryAdmissionHelperInternal_(const Key& key, const uint32_t& source_edge_idx, const MetadataUpdateRequirement& metadata_update_requirement, const DirectoryInfo& directory_info, const ExtraCommonMsghdr& extra_common_msghdr) const;
        bool afterDirectoryEvictionHelperInternal_(const Key& key, const uint32_t& source_edge_idx, const MetadataUpdateRequirement& metadata_update_requirement, const DirectoryInfo& directory_info, const CollectedPopularity& collected_popularity, const bool& is_global_cached, Edgeset& best_placement_edgeset, bool& need_hybrid_fetching, UdpMsgSocketServer* recvrsp_socket_server_ptr, const NetworkAddr& recvrsp_source_addr, BandwidthUsage& total_bandwidth_usage, EventList& event_list, const ExtraCommonMsghdr& extra_common_msghdr, const bool& is_background) const; // Return if edge is finished
        bool afterWritelockAcquireHelperInternal_(const Key& key, const uint32_t& source_edge_idx, const CollectedPopularity& collected_popularity, const bool& is_global_cached, const bool& is_source_cached, UdpMsgSocketServer* recvrsp_socket_server_ptr, const NetworkAddr& recvrsp_source_addr, BandwidthUsage& total_bandwidth_usage, EventList& event_list, const ExtraCommonMsghdr& extra_common_msghdr) const; // Return if edge is finished
        bool afterWritelockReleaseHelperInternal_(const Key& key, const uint32_t& source_edge_idx, const CollectedPopularity& collected_popularity, const bool& is_source_cached, Edgeset& best_placement_edgeset, bool& need_hybrid_fetching, UdpMsgSocketServer* recvrsp_socket_server_ptr, const NetworkAddr& recvrsp_source_addr, BandwidthUsage& total_bandwidth_usage, EventList& event_list, const ExtraCommonMsghdr& extra_common_msghdr) const; // Return if edge is finished
