...
namespace covered
{
    ...
    void CacheServerVictimFetchProcessor::start()
    {
        ...
        // Notify edge cache server that the current edge cache server victim fetch processor has finished initialization
        cache_server_victim_fetch_processor_param_ptr_->markFinishInitialization();
    ...
    bool CacheServerVictimFetchProcessor::processVictimFetchRequest_(MessageBase* control_request_ptr, const NetworkAddr& recvrsp_dst_addr)
    {
        ...
        total_bandwidth_usage.update(BandwidthUsage(0, covered_victim_fetch_request_ptr->getMsgBandwidthSize(), 0, 0, 1, 0));
