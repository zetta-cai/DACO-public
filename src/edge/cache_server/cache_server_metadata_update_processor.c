...
namespace covered
{
    ...
    void CacheServerMetadataUpdateProcessor::start()
    {
        ...
        // Notify edge cache server that the current edge cache server metadata update processor has finished initialization
        cache_server_metadata_update_processor_param_ptr_->markFinishInitialization();
    ...
    bool CacheServerMetadataUpdateProcessor::processMetadataUpdateRequest_(MessageBase* control_request_ptr, const NetworkAddr& recvrsp_dst_addr)
    {
        ...
        checkPointers_();
        CacheServerBase* tmp_cache_server_ptr = cache_server_metadata_update_processor_param_ptr_->getCacheServerPtr();
        ...
        total_bandwidth_usage.update(BandwidthUsage(0, covered_metadata_update_request_ptr->getMsgBandwidthSize(), 0, 0, 1, 0, covered_metadata_update_request_ptr->getMessageType(), covered_metadata_update_request_ptr->getVictimSyncsetBytes()));
