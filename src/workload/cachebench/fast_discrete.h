/*
 * WorkloadGenerator: refer to lib/CacheLib/cachelib/cachebench/workload/FastDiscrete.h.
 *
 * Hack to fix an integer overflow bug (convert -1 to uint64_t).
 * 
...
class NormalDistribution final : public Distribution {
 public:
  ...
  size_t operator()(std::mt19937_64& gen) override {
    ...
    XDCHECK_GE(v, left_);
    XDCHECK_LE(v, right_);
    return facebook::cachelib::util::narrow_cast<size_t>(v);
  }
 ...
 public:
  ...
  FastDiscreteDistribution(size_t left,
    ...
    for (auto it = buckets.begin(); it != buckets.end(); it++) {
      if (*it == 0) // Siyuan: *it == 0 means that no object in the current bucket
      {
        insideBucketDistributions_.emplace_back(0, 0);
      }
      else
      {
