...
namespace covered
{
    ...
    CoveredFghybridDirectoryEvictResponse::CoveredFghybridDirectoryEvictResponse(const Key& key, const bool& is_being_written, const VictimSyncset& victim_syncset, const Edgeset& edgeset, const uint32_t& source_index, const NetworkAddr& source_addr, const BandwidthUsage& bandwidth_usage, const EventList& event_list, const ExtraCommonMsghdr& extra_common_msghdr) : KeyByteVictimsetEdgesetMessage(key, static_cast<uint8_t>(is_being_written), victim_syncset, edgeset, MessageType::kCoveredFghybridDirectoryEvictResponse, source_index, source_addr, bandwidth_usage, event_list, extra_common_msghdr)
    {
    }
