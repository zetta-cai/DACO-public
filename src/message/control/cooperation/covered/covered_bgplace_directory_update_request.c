...
namespace covered
{
    ...
    CoveredBgplaceDirectoryUpdateRequest::CoveredBgplaceDirectoryUpdateRequest(const Key& key, const bool& is_valid_directory_exist, const DirectoryInfo& directory_info, const VictimSyncset& victim_syncset, const uint32_t& source_index, const NetworkAddr& source_addr, const ExtraCommonMsghdr& extra_common_msghdr) : KeyValidityDirectoryCollectpopVictimsetMessage(key, is_valid_directory_exist, directory_info, victim_syncset, MessageType::kCoveredBgplaceDirectoryUpdateRequest, source_index, source_addr, BandwidthUsage(), EventList(), extra_common_msghdr)
    {
    }

    CoveredBgplaceDirectoryUpdateRequest::CoveredBgplaceDirectoryUpdateRequest(const Key& key, const bool& is_valid_directory_exist, const DirectoryInfo& directory_info, const CollectedPopularity& collected_popularity, const VictimSyncset& victim_syncset, const uint32_t& source_index, const NetworkAddr& source_addr, const ExtraCommonMsghdr& extra_common_msghdr) : KeyValidityDirectoryCollectpopVictimsetMessage(key, is_valid_directory_exist, directory_info, collected_popularity, victim_syncset, MessageType::kCoveredBgplaceDirectoryUpdateRequest, source_index, source_addr, BandwidthUsage(), EventList(), extra_common_msghdr)
    {
    }
