...
namespace covered
{
    ...
    CoveredDirectoryLookupResponse::CoveredDirectoryLookupResponse(const Key& key, const bool& is_being_written, const bool& is_valid_directory_exist, const DirectoryInfo& directory_info, const VictimSyncset& victim_syncset, const uint32_t& source_index, const NetworkAddr& source_addr, const EventList& event_list, const bool& skip_propagation_latency) : KeyWriteflagValidityDirectoryVictimsetMessage(key, is_being_written, is_valid_directory_exist, directory_info, victim_syncset, MessageType::kCoveredDirectoryLookupResponse, source_index, source_addr, EventList(), skip_propagation_latency)
    {
    }
