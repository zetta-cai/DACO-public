...
namespace covered
{
    ...
    LocalPutResponse::LocalPutResponse(const Key& key, const Hitflag& hitflag, const uint32_t& source_index, const NetworkAddr& source_addr, const EventList& event_list) : KeyByteMessage(key, static_cast<uint8_t>(hitflag), MessageType::kLocalPutResponse, source_index, source_addr, event_list)
    {
    }
