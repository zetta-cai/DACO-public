/*
 * KeyVictimsetWriteflagValidityDirectoryMessage: the base class for messages each with a key, a write flag, a validity boolean, and a directory information.
 *
...
#ifndef KEY_VICTIMSET_WRITEFLAG_VALIDITY_DIRECTORY_MESSAGE_H
#define KEY_VICTIMSET_WRITEFLAG_VALIDITY_DIRECTORY_MESSAGE_H
...
namespace covered
{
    class KeyVictimsetWriteflagValidityDirectoryMessage : public MessageBase
    {
    public:
        KeyVictimsetWriteflagValidityDirectoryMessage(const Key& key, const VictimSyncset& victim_syncset, const bool& is_being_written, const bool& is_valid_directory_exist, const DirectoryInfo& directory_info, const MessageType& message_type, const uint32_t& source_index, const NetworkAddr& source_addr, const EventList& event_list, const bool& skip_propagation_latency);
        KeyVictimsetWriteflagValidityDirectoryMessage(const DynamicArray& msg_payload);
        virtual ~KeyVictimsetWriteflagValidityDirectoryMessage();
