...
namespace covered
{
    ...
    uint32_t KeyValueHitflagVictimsetEdgesetMessage::getVictimSyncsetBytes() const
    {
        checkIsValid_();
    ...
    uint32_t KeyValueHitflagVictimsetEdgesetMessage::getMsgBandwidthSizeInternal_() const
    {
        // key payload + ideal value content size + hitflag + victim syncset payload + edgeset payload
        uint32_t msg_bandwidth_size = key_.getKeyPayloadSize() + value_.getValuesize() + sizeof(uint8_t) + victim_syncset_.getVictimSyncsetPayloadSize() + edgeset_.getEdgesetPayloadSize();
