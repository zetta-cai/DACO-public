...
namespace covered
{
    ...
    void ClientWorkerWrapper::processLocalResponse_(const WorkloadItem& workload_item, const DynamicArray& local_response_msg_payload, const uint32_t& rtt_us, const bool& is_stresstest_phase, const bool& is_dynamic_mapped)
    {
        ...
        local_response_bandwidth_usage.update(BandwidthUsage(client_edge_local_rsp_bandwidth_bytes, 0, 0, 1, 0, 0, local_response_ptr->getMessageType(), local_response_ptr->getVictimSyncsetBytes())); // Get total bandwidth usage for received local response
