    ...
    void EdgeCLI::addCliParameters_()
    {
        if (!is_add_cli_parameters_)
        {
            ...
            argument_desc_.add_options()
                ("cache_name", boost::program_options::value<std::string>()->default_value(DEFAULT_CACHE_NAME), "cache name (e.g., arc, cachelib, fifo, frozenhot, glcache, lrb, lruk, gdsize, gdsf, lfuda, lfu, lhd, lru, s3fifo, segcache, sieve, slru, and covered)")
