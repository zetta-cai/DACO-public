...
namespace covered
{
    ...
    void EdgeCLI::addCliParameters_()
    {
        if (!is_add_cli_parameters_)
        {
            ...
            std::string cache_name_descstr = "cache name (see src/common/util.c for supported cache names)";
    ...
    {
        if (!Util::isSingleNodeCache(cache_name_) && !Util::isCooperativeCache(cache_name_) && !Util::isSharkLikeCache(cache_name_) && !Util::isMagnetLikeCache(cache_name_))
        {
            std::ostringstream oss;
