...
namespace covered
{
    ...
    const uint64_t EdgeCLI::DEFAULT_COVERED_LOCAL_UNCACHED_LRU_MAX_MB = 1;
    ...
    EdgeCLI::EdgeCLI() : WorkloadCLI(), EdgescaleCLI(), PropagationCLI(), is_add_cli_parameters_(false), is_set_param_and_config_(false), is_dump_cli_parameters_(false), is_create_required_directories_(false), is_to_cli_string_(false)
    {
        ...
        covered_local_uncached_lru_max_bytes_ = 0;
    ...
    uint64_t EdgeCLI::getCoveredLocalUncachedLruMaxBytes() const
    {
        return covered_local_uncached_lru_max_bytes_;
    }
    ...
    std::string EdgeCLI::toCliString()
    {
        ...
        if (!is_to_cli_string_)
        {
            ...
            if (cache_name_ == Util::COVERED_CACHE_NAME)
            {
                ...
                if (covered_local_uncached_lru_max_bytes_ != MB2B(DEFAULT_COVERED_LOCAL_UNCACHED_LRU_MAX_MB))
                {
                    oss << " -- covered_local_uncached_lru_max_mb " << B2MB(covered_local_uncached_lru_max_bytes_);
                }
    ...
    void EdgeCLI::addCliParameters_()
    {
        if (!is_add_cli_parameters_)
        {
            ...
            std::string cache_name_descstr = "cache name (e.g., " + Util::ARC_CACHE_NAME + ", " + Util::EXTENDED_ARC_CACHE_NAME + ", " + Util::BESTGUESS_CACHE_NAME + ", " + Util::CACHELIB_CACHE_NAME + ", " + Util::EXTENDED_CACHELIB_CACHE_NAME + ", " + Util::FIFO_CACHE_NAME + ", " + Util::EXTENDED_FIFO_CACHE_NAME + ", " + Util::FROZENHOT_CACHE_NAME + ", " + Util::EXTENDED_FROZENHOT_CACHE_NAME + ", " + Util::GLCACHE_CACHE_NAME + ", " + Util::EXTENDED_GLCACHE_CACHE_NAME + ", " + Util::LRB_CACHE_NAME + ", " + Util::EXTENDED_LRB_CACHE_NAME + ", " + Util::LRUK_CACHE_NAME + ", " + Util::EXTENDED_LRUK_CACHE_NAME + ", " + Util::GDSIZE_CACHE_NAME + ", " + Util::EXTENDED_GDSIZE_CACHE_NAME + ", " + Util::GDSF_CACHE_NAME + ", " + Util::EXTENDED_GDSF_CACHE_NAME + ", " + Util::LFUDA_CACHE_NAME + ", " + Util::EXTENDED_LFUDA_CACHE_NAME + ", " + Util::LFU_CACHE_NAME + ", " + Util::EXTENDED_LFU_CACHE_NAME + ", " + Util::LHD_CACHE_NAME + ", " + Util::EXTENDED_LHD_CACHE_NAME + ", " + Util::LRU_CACHE_NAME + ", " + Util::EXTENDED_LRU_CACHE_NAME + ", " + Util::MAGNET_CACHE_NAME + ", " + Util::S3FIFO_CACHE_NAME + ", " + Util::EXTENDED_S3FIFO_CACHE_NAME + ", " + Util::SEGCACHE_CACHE_NAME + ", " + Util::EXTENDED_SEGCACHE_CACHE_NAME + ", " + Util::SIEVE_CACHE_NAME + ", " + Util::EXTENDED_SIEVE_CACHE_NAME + ", " + Util::SLRU_CACHE_NAME + ", " + Util::EXTENDED_SLRU_CACHE_NAME + ", " + Util::WTINYLFU_CACHE_NAME + ", " + Util::EXTENDED_WTINYLFU_CACHE_NAME + ", and " + Util::COVERED_CACHE_NAME + ")";
            std::string hash_name_descstr = "the type of consistent hashing for DHT (e.g., " + Util::MMH3_HASH_NAME + ")";
            ...
            argument_desc_.add_options()
                ...
                ("covered_local_uncached_lru_max_mb", boost::program_options::value<uint64_t>()->default_value(DEFAULT_COVERED_LOCAL_UNCACHED_LRU_MAX_MB), "the maximum memory usage for local uncached LRU in units of MiB (only used for COVERED if enabled)")
    ...
    void EdgeCLI::setParamAndConfig_(const std::string& main_class_name)
    {
        if (!is_set_param_and_config_)
        {
            ...
            uint64_t covered_local_uncached_lru_max_bytes = MB2B(argument_info_["covered_local_uncached_lru_max_mb"].as<uint64_t>()); // In units of bytes
            ...
            if (cache_name == Util::COVERED_CACHE_NAME)
            {
                ...
                if (capacity_bytes * Config::getCoveredLocalUncachedLruMaxRatio() >= covered_local_uncached_lru_max_bytes)
                {
                    covered_local_uncached_lru_max_bytes_ = covered_local_uncached_lru_max_bytes;
                }
                else
                {
                    covered_local_uncached_lru_max_bytes_ = capacity_bytes * Config::getCoveredLocalUncachedLruMaxRatio();
                }
    ...
    void EdgeCLI::verifyAndDumpCliParameters_(const std::string& main_class_name)
    {
        if (!is_dump_cli_parameters_)
        {
            ...
            if (cache_name_ == Util::COVERED_CACHE_NAME)
            {
                ...
                oss << std::endl << "Covered local uncached lru max bytes: " << covered_local_uncached_lru_max_bytes_ << std::endl;
    ...
    void EdgeCLI::verifyIntegrity_() const
    {
        ...
        if (cache_name_ == Util::COVERED_CACHE_NAME)
        {
            ...
            assert(covered_local_uncached_lru_max_bytes_ > 0 && covered_local_uncached_lru_max_bytes_ < capacity_bytes);
