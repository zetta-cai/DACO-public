...
namespace covered
{
    ...
    void ClientCLI::verifyAndDumpCliParameters_(const std::string& main_class_name)
    {
        if (!is_dump_cli_parameters_)
        {
            ...
            oss << "Max workload loadcnt scale for evaluation: " << max_eval_workload_loadcnt_scale_ << std::endl;
    ...
    void ClientCLI::verifyIntegrity_(const std::string& main_class_name) const
    {
        ...
        if (main_class_name == Util::TRACE_PREPROCESSOR_MAIN_NAME)
        {
            ...
            {
                assert(perclient_opcnt_ == 0); // Set by setParamAndConfig_()
            }
            else
            {
                std::ostringstream oss;
        ...
        // Total workload loadcnt should > total warmup reqcnt
        assert(max_eval_workload_loadcnt_scale_ > warmup_reqcnt_scale_);
