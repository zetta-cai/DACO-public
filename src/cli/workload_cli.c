...
namespace covered
{
    ...
    void WorkloadCLI::addCliParameters_()
    {
        if (!is_add_cli_parameters_)
        {
            ...
            // Util::AKAMAI_WEB_WORKLOAD_NAME + ", " + Util::AKAMAI_VIDEO_WORKLOAD_NAME + ", " + Util::WIKIPEDIA_IMAGE_WORKLOAD_NAME + ", " + Util::WIKIPEDIA_TEXT_WORKLOAD_NAME + ", " + Util::FBPHOTO_WORKLOAD_NAME + ", " + Util::ZETA_WIKIPEDIA_IMAGE_WORKLOAD_NAME + ", " + Util::ZETA_WIKIPEDIA_TEXT_WORKLOAD_NAME + ", " + Util::ZETA_TENCENT_PHOTO1_WORKLOAD_NAME + ", " + Util::ZETA_TENCENT_PHOTO2_WORKLOAD_NAME
            std::string workload_name_descstr = "workload name (e.g., " + Util::FACEBOOK_WORKLOAD_NAME + ", " + Util::ZIPF_FACEBOOK_WORKLOAD_NAME + ", " + Util::ZIPF_WIKIPEDIA_IMAGE_WORKLOAD_NAME + ", " + Util::ZIPF_WIKIPEDIA_TEXT_WORKLOAD_NAME + ", " + Util::ZIPF_TENCENT_PHOTO1_WORKLOAD_NAME + ", " + Util::ZIPF_TENCENT_PHOTO2_WORKLOAD_NAME + ", " + Util::ZIPF_TWITTERKV2_WORKLOAD_NAME + ", and " + Util::ZIPF_TWITTERKV4_WORKLOAD_NAME + ")";
    ...
    void WorkloadCLI::setParamAndConfig_(const std::string& main_class_name)
    {
        if (!is_set_param_and_config_)
        {
            ...
            else if (workload_name == Util::FBPHOTO_WORKLOAD_NAME) // (OBSOLETE due to not open-sourced and and hence NO total frequency information for probability calculation and curvefitting)
            {
    ...
    {
        // OBSOLETE: self-reproduced Facebook photo caching trace is NOT open-sourced and NO probabilities for curvefitting; replayed traces cannot get reasonable results due to incorrect trace partitioning without geographical information
        // workload_name_ != Util::AKAMAI_WEB_WORKLOAD_NAME && workload_name_ != Util::AKAMAI_VIDEO_WORKLOAD_NAME && workload_name_ != Util::WIKIPEDIA_IMAGE_WORKLOAD_NAME && workload_name_ != Util::WIKIPEDIA_TEXT_WORKLOAD_NAME && workload_name_ != Util::FBPHOTO_WORKLOAD_NAME && workload_name_ != Util::ZETA_WIKIPEDIA_IMAGE_WORKLOAD_NAME && workload_name_ != Util::ZETA_WIKIPEDIA_TEXT_WORKLOAD_NAME && workload_name_ != Util::ZETA_TENCENT_PHOTO1_WORKLOAD_NAME && workload_name_ != Util::ZETA_TENCENT_PHOTO2_WORKLOAD_NAME
        if (workload_name_ != Util::FACEBOOK_WORKLOAD_NAME && workload_name_ != Util::ZIPF_FACEBOOK_WORKLOAD_NAME && workload_name_ != Util::ZIPF_WIKIPEDIA_IMAGE_WORKLOAD_NAME && workload_name_ != Util::ZIPF_WIKIPEDIA_TEXT_WORKLOAD_NAME && workload_name_ != Util::ZIPF_TENCENT_PHOTO1_WORKLOAD_NAME && workload_name_ != Util::ZIPF_TENCENT_PHOTO2_WORKLOAD_NAME && workload_name_ != Util::ZIPF_TWITTERKV2_WORKLOAD_NAME && workload_name_ != Util::ZIPF_TWITTERKV4_WORKLOAD_NAME)
        {
            std::ostringstream oss;
    ...
    void WorkloadCLI::verifyIntegrity_(const std::string& main_class_name) const
    {
        if (main_class_name == Util::TRACE_PREPROCESSOR_MAIN_NAME)
        {
            ...
            {
                std::ostringstream oss;
                oss << "workload " << workload_name_ << " is NOT single-node trace to be replayed and NO need to run trace preprocessor!";
                Util::dumpErrorMsg(kClassName, oss.str());
                exit(1);
            }
        }
