...
int main(int argc, char **argv) {
    ...
    supported_component_names.push_back(covered::Util::SIMULATOR_MAIN_NAME); // NOTE: simulator CLI also covers client/edge/cloud/evaluator/total_statistics_loader CLI
    ...
    else if (tmp_component_name == covered::Util::DATASET_LOADER_MAIN_NAME)
    {
        // (1) Parse and process CLI parameters
        covered::DatasetLoaderCLI dataset_loader_cli(new_argc, new_argv);

        // (2) Print CLI string for different components

        // Get CLI string of different componenets
        std::string dataset_loader_cli_string = dataset_loader_cli.toCliString();
        dataset_loader_cli.clearIsToCliString();
