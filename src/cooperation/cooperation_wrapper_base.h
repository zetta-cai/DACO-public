...
namespace covered
{
    ...
    public:
        ...
        virtual std::list<std::pair<Key, DirinfoSet>> getLocalBeaconedVictimsFromVictimSyncset(const VictimSyncset& victim_syncset) const = 0; // NOTE: all edge cache/beacon/invalidation servers will access cooperation wrapper to get content directory information for local beaconed victims from received victim syncset
        virtual std::list<std::pair<Key, DirinfoSet>> getLocalBeaconedVictimsFromCacheinfos(const std::list<VictimCacheinfo>& victim_cacheinfos) const = 0;
