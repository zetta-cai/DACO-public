...
int main(int argc, char **argv) {
    ...
    // NOTE: dynamic workload parameters are only used by clients to generate workload items during stresstest phase, rather than by trace preprocessor in preprocessing phase
    const std::string workload_pattern_name = trace_preprocessor_cli.getWorkloadPatternName();
    const uint32_t dynamic_change_period = trace_preprocessor_cli.getDynamicChangePeriod();
    const uint32_t dynamic_change_keycnt = trace_preprocessor_cli.getDynamicChangeKeycnt();
