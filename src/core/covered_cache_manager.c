...
namespace covered
{
    ...
    CoveredCacheManager::CoveredCacheManager(const uint32_t& edge_idx, const uint32_t& edgecnt, const uint32_t& peredge_synced_victimcnt, const uint32_t& peredge_monitored_victimsetcnt, const uint64_t& popularity_aggregation_capacity_bytes, const double& popularity_collection_change_ratio, const uint32_t& topk_edgecnt) : edge_idx_(edge_idx), topk_edgecnt_(topk_edgecnt), directory_cacher_(edge_idx, popularity_collection_change_ratio)
    {
        ...
        victim_tracker_ptr_ = new VictimTracker(edge_idx, edgecnt, peredge_synced_victimcnt, peredge_monitored_victimsetcnt);
                ...
                else
                {
                    ...
                    for (std::unordered_map<uint32_t, bool>::iterator iter_for_response = acked_flags.begin(); iter_for_response != acked_flags.end(); iter_for_response++)
                    {
                        if (iter_for_response->first == tmp_edge_idx) // Match an edge node requiring victim fetching
                        {
                            ...
                            victim_fetch_response_bandwidth_usage.update(BandwidthUsage(0, cross_edge_victim_fetch_rsp_bandwidth_bytes, 0, 0, 1, 0, control_response_ptr->getMessageType(), control_response_ptr->getVictimSyncsetBytes()));
    ...
    void CoveredCacheManager::processVictimFetchResponse_(const MessageBase* control_response_ptr, const EdgeWrapperBase* edge_wrapper_ptr, std::list<std::pair<uint32_t, std::list<VictimCacheinfo>>>& extra_peredge_victim_cacheinfos, std::list<std::pair<Key, DirinfoSet>>& extra_perkey_victim_dirinfoset) const
    {
        ...
        // (1) Update extra_peredge_victim_cacheinfos
        bool with_complete_victim_syncset = false;
        const std::list<VictimCacheinfo>* fetched_victim_cacheinfos_ptr = victim_fetchset.getLocalSyncedVictimsPtr(with_complete_victim_syncset);
        assert(with_complete_victim_syncset == true); // NOTE: extra fetched victim cacheinfos in victim fetchset MUST be complete
        extra_peredge_victim_cacheinfos.push_back(std::pair<uint32_t, std::list<VictimCacheinfo>>(source_edge_idx, *fetched_victim_cacheinfos_ptr));

        // (2) Update extra_perkey_victim_dirinfoset
        with_complete_victim_syncset = false;
        const std::list<std::pair<Key, DirinfoSet>>* neighbor_beaconed_neighbor_fetched_victim_dirinfosets_ptr = victim_fetchset.getLocalBeaconedVictimsPtr(with_complete_victim_syncset); // Neighbor beaconed ones of neighbor fetched victims
        assert(with_complete_victim_syncset == true); // NOTE: extra fetched victim dirinfo sets in victim fetchset MUST be complete

        // (2-1) Insert neighbor beaconed neighbor fetched victims
        for (std::list<std::pair<Key, DirinfoSet>>::const_iterator victim_dirinfosets_const_iter = neighbor_beaconed_neighbor_fetched_victim_dirinfosets_ptr->begin(); victim_dirinfosets_const_iter != neighbor_beaconed_neighbor_fetched_victim_dirinfosets_ptr->end(); victim_dirinfosets_const_iter++)
        {
