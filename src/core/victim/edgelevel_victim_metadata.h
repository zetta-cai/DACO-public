...
namespace covered
{
    ...
    public:
        ...
        void findVictimsForObjectSize(const uint32_t& cur_edge_idx, const ObjectSize& object_size, std::unordered_map<Key, Edgeset, KeyHasher>& pervictim_edgeset, std::unordered_map<Key, std::list<VictimCacheinfo>, KeyHasher>& pervictim_cacheinfos, std::unordered_map<uint32_t, std::list<Key>>& peredge_synced_victimset, std::unordered_map<uint32_t, std::list<Key>>& peredge_fetched_victimset, Edgeset& victim_fetch_edgeset, const std::list<VictimCacheinfo>& extra_victim_cacheinfos) const;
    ...
    private:
        ...
        void updatePeredgeVictimset_(std::unordered_map<uint32_t, std::list<Key>>& peredge_victimset, const uint32_t& edge_idx, const Key& victim_key) const;
