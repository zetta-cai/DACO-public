...
namespace covered
{
    ...
    VictimSyncset VictimTracker::getLocalVictimSyncset(const uint64_t& latest_local_cache_margin_bytes) const
    {
        checkPointers_();

        // Acquire a read lock to get local victim syncset atomically
        std::string context_name = "VictimTracker::getLocalVictimSyncset()";
        rwlock_for_victim_tracker_->acquire_lock_shared(context_name);

        VictimSyncset local_victim_syncset = getVictimSyncset_(edge_idx_);
        ...
        local_victim_syncset.setCacheMarginBytes(latest_local_cache_margin_bytes);
        assert(local_victim_syncset.isComplete());

        return local_victim_syncset;
    }
    ...
    bool VictimTracker::replacePrevVictimSyncset(const uint32_t& dst_edge_idx_for_compression, const VictimSyncset& current_victim_syncset, VictimSyncset& prev_victim_syncset)
    {
        checkPointers_();
        ...
        // Acquire a write lock to update prev victim syncset
        std::string context_name = "VictimTracker::replacePrevVictimSyncset()";
        rwlock_for_victim_tracker_->acquire_lock(context_name);
    ...
    {
        // NOTE: NO need to acquire a read lock which has been done in getLocalVictimSyncset() and getVictimSyncset()
