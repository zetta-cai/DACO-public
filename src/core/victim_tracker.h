...
namespace covered
{
    ...
    {
    public:
        VictimTracker(const uint32_t& edge_idx, const uint32_t& edgecnt, const uint32_t& peredge_synced_victimcnt, const uint32_t& peredge_monitored_victimsetcnt);
        ~VictimTracker();
        ...
        DeltaReward calcEvictionCost(const EdgeWrapperBase* edge_wrapper_ptr, const ObjectSize& object_size, const Edgeset& placement_edgeset, std::list<std::pair<uint32_t, std::list<Key>>>& placement_peredge_synced_victimset, std::list<std::pair<uint32_t, std::list<Key>>>& placement_peredge_fetched_victimset, Edgeset& victim_fetch_edgeset, const std::list<std::pair<uint32_t, std::list<VictimCacheinfo>>>& extra_peredge_victim_cacheinfos = std::list<std::pair<uint32_t, std::list<VictimCacheinfo>>>(), const std::list<std::pair<Key, DirinfoSet>>& extra_perkey_victim_dirinfoset = std::list<std::pair<Key, DirinfoSet>>()) const;
    ...
    private:
        ...
        typedef std::list<std::pair<Key, VictimDirinfo>> perkey_victim_dirinfo_t; // NOT use unordered map due to limited number of victims
        typedef std::vector<VictimsyncMonitor> peredge_victimsync_monitor_t;
        ...
        void replaceVictimDirinfoSets_(const std::list<std::pair<Key, DirinfoSet>>& beaconed_synced_victim_dirinfosets, const bool& is_local_beaconed); // Replace dirinfoset in existing VictimDirinfo if any of each local/neighbor beaconed victim
