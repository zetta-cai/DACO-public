...
namespace covered
{
    ...
    {
    public:
        CoveredCacheManager(const uint32_t& edge_idx, const uint32_t& edgecnt, const uint32_t& peredge_synced_victimcnt, const uint32_t& peredge_monitored_victimsetcnt, const uint64_t& popularity_aggregation_capacity_bytes, const double& popularity_collection_change_ratio, const uint32_t& topk_edgecnt);
        ~CoveredCacheManager();
        ...
        void updateVictimTrackerForLocalCacheMarginBytes(const uint64_t& local_cache_margin_bytes); // ONLY update cache margin bytes of local synced victims
        void updateVictimTrackerForLocalSyncedVictims(const uint64_t& local_cache_margin_bytes, const std::list<VictimCacheinfo>& local_synced_victim_cacheinfos, const CooperationWrapperBase* cooperation_wrapper_ptr); // Update cache margin bytes, victim cacheinfos, and victim dirinfos of local synced victims
    ...
    private:
        ...
        void sendVictimFetchRequest_(const uint32_t& dst_edge_idx_for_compression, const ObjectSize& object_size, const EdgeWrapperBase* edge_wrapper_ptr, const NetworkAddr& recvrsp_source_addr, const NetworkAddr& edge_cache_server_recvreq_dst_addr, const ExtraCommonMsghdr& extra_common_msghdr) const;
