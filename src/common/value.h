 ...
 * (ii) System performance bottleneck of geo-distributed tiered storage is propagation latency instead of emission latency for most objects and thus most hot objects -> NOT affect evaluation performance results;
 * (ii-i) -> Actually, if emission latency is bottleneck, both edge caching and cooperative caching make NO sense, which is out of our scope;
 * (ii-ii) -> NOTE: as we do not simulate propagation latency during warmup, reducing emission latency can significantly speedup warmup;
