...
namespace covered
{
    ...
    public:
        ...
        bool isInvalidated(const uint32_t& key) const; // For normal requests (e.g., local/redirected requests)
        void invalidate(const uint32_t& key); // For control requests (e.g., invalidation and admission/eviction)
        void validate(const uint32_t& key); // For control requests (e.g., invalidation and admission/eviction)
    ...
    private:
        ...
        // NOTE: ONLY write invalidity_map_ for control requests (e.g., invalidation and admission/eviction), while just read it for normal requests (local/redirected)
