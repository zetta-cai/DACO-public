/*
 * SlruLocalCache: local edge cache with SLRU policy (the same repo as S3-FIFO in https://github.com/Thesys-lab/sosp23-s3fifo).
 * 
 * By Siyuan Sheng (2024.01.20).
 */

#ifndef SLRU_LOCAL_CACHE_H
#define SLRU_LOCAL_CACHE_H
...
namespace covered
{
    class SlruLocalCache : public LocalCacheBase
    {
    public:
        SlruLocalCache(const EdgeWrapperBase* edge_wrapper_ptr, const uint32_t& edge_idx, const uint64_t& capacity_bytes);
        virtual ~SlruLocalCache();
    ...
    private:
        ...
        SlruCachePolicy<Key, Value, KeyHasher>* slru_cache_ptr_; // Data and metadata for local edge cache
    };
}
