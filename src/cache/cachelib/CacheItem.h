...
class CACHELIB_PACKED_ATTR CacheItem {
 public:
  ...
  using ChainedItem = CacheChainedItem<CacheTrait>; // covered::CacheChainedItem
   ...
   */
  using ReadHandle = facebook::cachelib::detail::ReadHandleImpl<CacheItem>;
  using WriteHandle = facebook::cachelib::detail::WriteHandleImpl<CacheItem>;
 ...
 protected:
   ...
   *  cachelib::facebook::cachelib::exception::RefcountOverflow
   *  cachelib::facebook::cachelib::exception::RefcountUnderflow
   */
  ...
  // @throw facebook::cachelib::exception::RefcountOverflow on ref count overflow
  FOLLY_ALWAYS_INLINE bool incRef() {
    try {
      return ref_.incRef();
    } catch (facebook::cachelib::exception::RefcountOverflow& e) {
      throw facebook::cachelib::exception::RefcountOverflow(
...
class CACHELIB_PACKED_ATTR CacheChainedItem : public CacheItem<CacheTrait> {
 public:
  using Item = CacheItem<CacheTrait>; // covered::CacheItem
  using ChainedItem = CacheChainedItem<CacheTrait>; // covered::CacheChainedItem
  using Payload = ChainedItemPayload<CacheTrait>; // covered::ChainedItemPayload
 ...
 protected:
  ...
  friend Payload;
  friend CacheAllocator<CacheTrait>; // covered::CacheAllocator
  template <typename Cache, typename Handle, typename Iter>
  friend class CacheChainedAllocs;
  template <typename Cache, typename Item>
  friend class CacheChainedItemIterator;
