...
bool CacheItem<CacheTrait>::isExpired() const noexcept {
  ...
  uint32_t currentTime = static_cast<uint32_t>(facebook::cachelib::util::getCurrentTimeSec());
...
bool CacheItem<CacheTrait>::extendTTL(std::chrono::seconds ttl) noexcept {
  return updateExpiryTime(facebook::cachelib::util::getCurrentTimeSec() + ttl.count());
}
