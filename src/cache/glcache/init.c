...
void cpp_init_global_params() {
  ...
  cpp_ofile_cmp_y = fopen("compare_y.txt", "w");
  fprintf(cpp_ofile_cmp_y,
...
void deinit_global_params() {
  if (cpp_ofile_cmp_y != NULL) {
    fclose(cpp_ofile_cmp_y);
  }
}

void cpp_check_params(GLCache_params_t *params) {
