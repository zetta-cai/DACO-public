/*
 * AdaptSizeLocalCache: local edge cache with AdaptSize (https://github.com/dasebe/webcachesim).
 * 
 * By Siyuan Sheng (2024.11.19).
 */

#ifndef ADAPTSIZE_LOCAL_CACHE_H
#define ADAPTSIZE_LOCAL_CACHE_H
...
namespace covered
{
    class AdaptSizeLocalCache : public LocalCacheBase
    {
    public:
        AdaptSizeLocalCache(const EdgeWrapperBase* edge_wrapper_ptr, const std::string& cache_name, const uint32_t& edge_idx, const uint64_t& capacity_bytes);
        virtual ~AdaptSizeLocalCache();
    ...
    private:
        ...
        AdaptSizeCache* adaptsize_cache_ptr_; // Data and metadata for local edge cache
    };
}
