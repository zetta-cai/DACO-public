/*
 * LacacheLocalCache: local edge cache with LA-Cache policy (https://github.com/GYan58/la-cache).
 * 
 * By Siyuan Sheng (2024.08.03).
 */

#ifndef LACACHE_LOCAL_CACHE_H
#define LACACHE_LOCAL_CACHE_H
...
namespace covered
{
    class LacacheLocalCache : public LocalCacheBase
    {
    public:
        LacacheLocalCache(const EdgeWrapperBase* edge_wrapper_ptr, const uint32_t& edge_idx, const uint64_t& capacity_bytes);
        virtual ~LacacheLocalCache();
    ...
    private:
        ...
        LACache* lacache_cache_ptr_; // Data and metadata for local edge cache
    };
}
