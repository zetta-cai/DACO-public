...
namespace covered
{
    ...
    public:
        ...
        void updateForNewlyGrouped(const Key& key, const Value& value); // For admission and getrsp/put/delreq w/ miss, update group-level metadata (both value-unrelated and value-related) for the key newly added into the current group (newly admitted/tracked for local cached/uncached)
        ...
        void updateValueStatsForInGroupKey(const Key& key, const Value& value, const Value& original_value); // For put/delreq w/ hit/miss and getrsp w/ invalid-hit, update group-level value-related metadata for the key already in the current group (i.e., already admitted/tracked objects for local cached/uncached)
