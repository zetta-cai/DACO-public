...
namespace covered
{
    ...
    public:
        ...
        bool updateNoValueStatsForExistingKey_p2p(const EdgeWrapperBase* edge_wrapper_ptr, const Key& key, const uint32_t& peredge_synced_victimcnt, const bool& is_redirected, const bool& is_global_cached, const uint32_t redirected_reward); // Return if affect local synced victims in victim tracker (always return false for local uncached metadata)
    ...
    private:
        ...
        perkey_metadata_list_iter_t updateNoValuePerkeyMetadata_p2p_(const perkey_lookup_table_iter_t& perkey_lookup_iter, const bool& is_redirected, const bool& is_global_cached, const uint32_t redirected_reward); // For get/put/delreq w/ hit/miss, update object-level value-unrelated metadata for existing key (i.e., already admitted/tracked objects for local cached/uncached); return updated KeyLevelMetadata
