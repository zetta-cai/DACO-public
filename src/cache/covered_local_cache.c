...
namespace covered
{
    ...
    void CoveredLocalCache::admitLocalCacheInternal_(const Key& key, const Value& value, const bool& is_neighbor_cached, bool& affect_victim_tracker, bool& is_successful, const uint64_t& miss_latency_us)
    {
        ...
        {
            std::ostringstream oss;
            oss << "key " << key.getKeyDebugstr() << " is already local cached before admitLocalCacheInternal_(), which may be caused by occasional duplicate placement notification due to NOT strong atomicity of CoveredCacheManager";
    ...
    bool CoveredLocalCache::getLocalCacheVictimKeysInternal_(std::unordered_set<Key, KeyHasher>& keys, std::list<VictimCacheinfo>& victim_cacheinfos, const uint64_t& required_size) const
    {
        ...
        while (conservative_victim_total_size < required_size) // Provide multiple victims based on required size for lazy victim fetching
        {
            ...
            Reward tmp_redirected_cached_reward_ = 0.0;
            ...
            if (is_least_popular_key_exist)
            {
                ...
                local_cached_metadata_.getPopularity(tmp_victim_key, tmp_local_cached_popularity, tmp_redirected_cached_popularity, tmp_redirected_cached_reward_);
                ...
                if (keys.find(tmp_victim_key) == keys.end())
                {
                    ...
                    VictimCacheinfo tmp_victim_info(tmp_victim_key, tmp_victim_object_size, tmp_local_cached_popularity, tmp_redirected_cached_popularity, tmp_local_reward, tmp_redirected_cached_reward_);
    ...
    void CoveredLocalCache::invokeCustomFunctionInternal_(const std::string& func_name, CacheCustomFuncParamBase* func_param_ptr)
    {
        ...
        if (func_name == UpdateIsNeighborCachedFlagFuncParam::FUNCNAME)
        {
            ...
            updateIsNeighborCachedInternal_(tmp_param_ptr->getKey(), tmp_param_ptr->isNeighborCached());            
        }
        else
        {
            std::ostringstream oss;
    ...
    void CoveredLocalCache::updateIsNeighborCachedInternal_(const Key& key, const bool& is_neighbor_cached)
    {
    ...
    void CoveredLocalCache::invokeConstCustomFunctionInternal_(const std::string& func_name, CacheCustomFuncParamBase* func_param_ptr) const
    {
        ...
        if (func_name == GetLocalSyncedVictimCacheinfosParam::FUNCNAME)
        {
            ...
            getLocalSyncedVictimCacheinfosFromLocalCacheInternal_(tmp_param_ptr->getVictimCacheinfosRef());
        ...
        else if (func_name == GetCollectedPopularityParam::FUNCNAME)
        {
            ...
            getCollectedPopularityFromLocalCacheInternal_(tmp_param_ptr->getKey(), tmp_param_ptr->getCollectedPopularityRef());

            // Object size checking
            const CollectedPopularity& collected_popularity_const_ref = tmp_param_ptr->getCollectedPopularityConstRef();
            if (collected_popularity_const_ref.isTracked())
            {
                const bool is_valid_objsize = isValidObjsize_(collected_popularity_const_ref.getObjectSize());
                assert(is_valid_objsize);
            }
        }
        else
    ...
    void CoveredLocalCache::getLocalSyncedVictimCacheinfosFromLocalCacheInternal_(std::list<VictimCacheinfo>& victim_cacheinfos) const
    {
        for (uint32_t least_popular_rank = 0; least_popular_rank < peredge_synced_victimcnt_; least_popular_rank++)
        {
            ...
            Popularity tmp_redirected_cached_popularity = 0.0;
            Reward tmp_local_reward = 0.0;
            Reward tmp_redirected_reward = 0.0;
            bool is_least_popular_key_exist = local_cached_metadata_.getLeastRewardKeyAndReward(least_popular_rank, tmp_victim_key, tmp_local_reward);
            ...
            if (is_least_popular_key_exist)
            {
                ...
                local_cached_metadata_.getPopularity(tmp_victim_key, tmp_local_cached_popularity, tmp_redirected_cached_popularity, tmp_redirected_reward);
                ...
                VictimCacheinfo tmp_victim_info(tmp_victim_key, tmp_victim_object_size, tmp_local_cached_popularity, tmp_redirected_cached_popularity, tmp_local_reward, tmp_redirected_reward);
                assert(tmp_victim_info.isComplete()); // NOTE: victim cacheinfos from local edge cache MUST be complete
                victim_cacheinfos.push_back(tmp_victim_info); // Add to the tail of the list
            }
            else
        ...
        assert(victim_cacheinfos.size() <= peredge_synced_victimcnt_);
