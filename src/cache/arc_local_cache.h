/*
 * ArcLocalCache: local edge cache with ARC policy (the same repo as S3-FIFO in https://github.com/Thesys-lab/sosp23-s3fifo).
 * 
 * By Siyuan Sheng (2024.01.20).
 */

#ifndef ARC_LOCAL_CACHE_H
#define ARC_LOCAL_CACHE_H

#include <string>

#include "cache/arc/arc_cache_policy.hpp"
#include "cache/local_cache_base.h"

namespace covered
{
    class ArcLocalCache : public LocalCacheBase
    {
    public:
        ArcLocalCache(const EdgeWrapperBase* edge_wrapper_ptr, const uint32_t& edge_idx, const uint64_t& capacity_bytes);
        virtual ~ArcLocalCache();
    ...
    private:
        ...
        ArcCachePolicy<Key, Value, KeyHasher>* arc_cache_ptr_; // Data and metadata for local edge cache
    };
}
