...
class LFUCachePolicy : public ICachePolicy<Key>
{
  public:
    ...
    typedef std::unordered_map<Key, lfu_iterator, KeyHasher>::iterator map_iterator_t;

    LFUCachePolicy() : ICachePolicy<Key>()
    {
        frequency_storage.resize(0);
        lfu_storage.resize(0);
        size_ = 0;
    }
  ...
  private:
    static const std::string kClassName("LFUCachePolicy");
