...
// #include "algorithm"
// #include "sys/time.h"
// Siyuan: use brackets instead of double-quotes for system headers
#include <thread>
#include <random>
#include <sys/mman.h>
#include <string.h>
...
#include <cassert>
//#include "stdlib.h"
#include <stdlib.h> // Siyuan: use brackets instead of double-quotes for system headers
...
namespace statistics{
    ...
    struct AvgStat{
      ...
      public:
        ...
        void add(double value){
          int i = covered::random::Random::GetTLSInstance()->Next() % NUM;
    ...
    struct MySet{
      ...
      public:
        ...
        double print_tail() {
          if(size_ == 0){
            // Siyuan: disable most dump information by default
            UTIL_PRINTF("no stat to print tail\n");
            fflush(stdout);
            return 100;
          }
          auto size = size_;
          ...
          // Siyuan: disable most dump information by default
          UTIL_PRINTF("Avg: %.3lf (stat size: %ld, real size_: %ld), median: %.3lf, p9999: %.3lf, p999: %.3lf, p99: %.3lf, p90: %.3lf\n", \
        ...
        double print_from_last_end(uint64_t& step) {
          ...
          step = size - cursor;
          // Siyuan: disable most dump information by default
          UTIL_PRINTF("Avg: %.3lf (stat size: %ld, size: %ld -> %ld)\n", \
