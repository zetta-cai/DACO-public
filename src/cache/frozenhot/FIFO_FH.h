...
#ifndef FIFO_PRTINF
#ifdef DEBUG_COVERED_incl_FIFO_CACHE_FH_H
#define FIFO_PRTINF(...) printf(__VA_ARGS__)
#else
#define FIFO_PRTINF(...)
#endif
#endif
...
bool FIFO_FHCache<TKey, TValue, THash>::construct_ratio(double FC_ratio) {
  ...
  FIFO_PRTINF("FC_size: %llu, DC_size: %llu\n", FC_bytes, DC_bytes);
  ...
  if(fail_bytes > 0)
    FIFO_PRTINF("fast hash insert num: %lu, fail bytes: %lu, dckv_used_bytes: %ld (FC_ratio: %.2lf)\n", 
        bytes, fail_bytes, dckv_used_bytes.load(), bytes*1.0/dckv_used_bytes.load());
  else
    FIFO_PRTINF("fast hash insert bytes: %lu, dckv_used_bytes: %ld (FC_ratio: %.2lf)\n", 
...
bool FIFO_FHCache<TKey, TValue, THash>::construct_tier() {
  ...
  //     count, m_size.load(), count*1.0/m_size.load());
  FIFO_PRTINF("fast hash insert bytes: %d, dckv_used_bytes: %ld (FC_ratio: %.2lf)\n", 
      bytes, dckv_used_bytes.load(), bytes*1.0/dckv_used_bytes.load());
  tier_ready = true;
  fast_hash_construct = false;
  return true;
...
void FIFO_FHCache<TKey, TValue, THash>::debug(int status) {
  if(status == 0){
    ...
    FIFO_PRTINF("Main LRU list Size: %d, m_size: %ld\n", count, m_size.load());
  }
  else if(status == 1){
    ...
    FIFO_PRTINF("Fast LRU list Size: %d, m_size: %ld\n", count, m_size.load());
  }
  else if(status == 2){
    std::unique_lock<ListMutex> lock(m_listMutex);
    ListNode* temp = m_head.m_next;
...
void FIFO_FHCache<TKey, TValue, THash>::deconstruct() {
  ...
  if(m_fast_head.m_next == &m_fast_tail){
    FIFO_PRTINF("no need to deconstruct!\n");
    fflush(stdout);
    return;
  }
...
// Siyuan: for fine-grained eviction
template <class TKey, class TValue, class THash>
bool FIFO_FHCache<TKey, TValue, THash>::findVictimKey(TKey& key)
{
#ifdef HANDLE_WRITE
  std::unique_lock<ListMutex> lock(m_listMutex);
  ListNode* moribund = m_tail.m_prev;
...
template <class TKey, class TValue, class THash>
bool FIFO_FHCache<TKey, TValue, THash>::evict(const TKey& key, TValue& value)
{
  // Check if it has already been deleted somehow
  // Usually node and hash item are deleted at the same time
  // rarely see this case
