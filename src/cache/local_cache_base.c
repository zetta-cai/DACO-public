...
namespace covered
{
    ...
    LocalCacheBase* LocalCacheBase::getLocalCacheByCacheName(const EdgeWrapperBase* edge_wrapper_ptr, const std::string& cache_name, const uint32_t& edge_idx, const uint64_t& capacity_bytes, const uint32_t& dataset_keycnt, const uint64_t& local_uncached_capacity_bytes, const uint64_t& local_uncached_lru_bytes, const uint32_t& peredge_synced_victimcnt)
    {
        ...
        else if (cache_name == Util::CACHELIB_CACHE_NAME || cache_name == Util::SHARK_EXTENDED_CACHELIB_CACHE_NAME || cache_name == Util::MAGNET_EXTENDED_CACHELIB_CACHE_NAME)
        {
            local_cache_ptr = new CachelibLocalCache(edge_wrapper_ptr, edge_idx, capacity_bytes);
        }
        else if (cache_name == Util::FIFO_CACHE_NAME || cache_name == Util::SHARK_EXTENDED_FIFO_CACHE_NAME || cache_name == Util::MAGNET_EXTENDED_FIFO_CACHE_NAME)
        {
            local_cache_ptr = new FifoLocalCache(edge_wrapper_ptr, edge_idx, capacity_bytes);
        }
        else if (cache_name == Util::FROZENHOT_CACHE_NAME || cache_name == Util::SHARK_EXTENDED_FROZENHOT_CACHE_NAME || cache_name == Util::MAGNET_EXTENDED_FROZENHOT_CACHE_NAME)
        {
            local_cache_ptr = new FrozenhotLocalCache(edge_wrapper_ptr, edge_idx, capacity_bytes);
        }
        else if (cache_name == Util::GLCACHE_CACHE_NAME || cache_name == Util::SHARK_EXTENDED_GLCACHE_CACHE_NAME || cache_name == Util::MAGNET_EXTENDED_GLCACHE_CACHE_NAME)
        {
        ...
        else if (cache_name == Util::LHD_CACHE_NAME || cache_name == Util::SHARK_EXTENDED_LHD_CACHE_NAME || cache_name == Util::MAGNET_EXTENDED_LHD_CACHE_NAME)
        {
            local_cache_ptr = new LhdLocalCache(edge_wrapper_ptr, edge_idx, capacity_bytes);
        }
        else if (cache_name == Util::LRB_CACHE_NAME || cache_name == Util::SHARK_EXTENDED_LRB_CACHE_NAME || cache_name == Util::MAGNET_EXTENDED_LRB_CACHE_NAME)
        {
            local_cache_ptr = new LrbLocalCache(edge_wrapper_ptr, edge_idx, capacity_bytes, dataset_keycnt);
        }
        else if (cache_name == Util::LRU_CACHE_NAME || cache_name == Util::SHARK_EXTENDED_LRU_CACHE_NAME || cache_name == Util::MAGNET_EXTENDED_LRU_CACHE_NAME)
        {
            local_cache_ptr = new LruLocalCache(edge_wrapper_ptr, edge_idx, capacity_bytes);
        }
        else if (cache_name == Util::S3FIFO_CACHE_NAME || cache_name == Util::SHARK_EXTENDED_S3FIFO_CACHE_NAME || cache_name == Util::MAGNET_EXTENDED_S3FIFO_CACHE_NAME)
        {
            local_cache_ptr = new S3fifoLocalCache(edge_wrapper_ptr, edge_idx, capacity_bytes);
        }
        else if (cache_name == Util::SEGCACHE_CACHE_NAME || cache_name == Util::SHARK_EXTENDED_SEGCACHE_CACHE_NAME || cache_name == Util::MAGNET_EXTENDED_SEGCACHE_CACHE_NAME)
        {
            local_cache_ptr = new SegcacheLocalCache(edge_wrapper_ptr, edge_idx, capacity_bytes);
        }
        else if (cache_name == Util::SIEVE_CACHE_NAME || cache_name == Util::SHARK_EXTENDED_SIEVE_CACHE_NAME || cache_name == Util::MAGNET_EXTENDED_SIEVE_CACHE_NAME)
        {
            local_cache_ptr = new SieveLocalCache(edge_wrapper_ptr, edge_idx, capacity_bytes);
        }
        else if (cache_name == Util::SLRU_CACHE_NAME || cache_name == Util::SHARK_EXTENDED_SLRU_CACHE_NAME || cache_name == Util::MAGNET_EXTENDED_SLRU_CACHE_NAME)
        {
            local_cache_ptr = new SlruLocalCache(edge_wrapper_ptr, edge_idx, capacity_bytes);
        }
        else if (cache_name == Util::WTINYLFU_CACHE_NAME || cache_name == Util::SHARK_EXTENDED_WTINYLFU_CACHE_NAME || cache_name == Util::MAGNET_EXTENDED_WTINYLFU_CACHE_NAME)
        {
