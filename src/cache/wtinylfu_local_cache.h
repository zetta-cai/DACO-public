/*
 * WTinylfuLocalCache: local edge cache with W-TinyLFU policy (the same repo as S3-FIFO in https://github.com/Thesys-lab/sosp23-s3fifo).
 * 
 * By Siyuan Sheng (2024.01.23).
 */

#ifndef WTINYLFU_LOCAL_CACHE_H
#define WTINYLFU_LOCAL_CACHE_H

#include <string>

#include "cache/wtinylfu/wtinylfu_cache_policy.hpp"
#include "cache/local_cache_base.h"

namespace covered
{
    class WTinylfuLocalCache : public LocalCacheBase
    {
    public:
        WTinylfuLocalCache(const EdgeWrapperBase* edge_wrapper_ptr, const uint32_t& edge_idx, const uint64_t& capacity_bytes);
        virtual ~WTinylfuLocalCache();
    ...
    private:
        ...
        WTinylfuCachePolicy<Key, Value, KeyHasher>* wtinylfu_cache_ptr_; // Data and metadata for local edge cache
    };
}
